 ...
 *
 * Providers register on `init`, which is the earliest hook guaranteed to run
 * AFTER activation has finished applying migrations — so this is where the
 * version dispatcher belongs, not in plugin/activation.php (which executes
 * before any table exists).
 *
 * The cost on a normal request is one autoloaded option read.
 */
