 ...
 * Deactivating a plugin must not strip roles from live user accounts.
 *
 * Capability alone is never authorisation: every trainer/user endpoint also runs
 * a resource-ownership check (see plans/03-backend.md#capability-is-not-authorisation).
 */
final class RoleProvider
{
    ...
    public static function install(): void
    {
        add_role(
            self::ROLE_USER,
            __('FitnessClub Member', 'fitnessclub'),
            array_fill_keys(self::USER_CAPS, true)
        );

        add_role(
            self::ROLE_TRAINER,
            __('FitnessClub Trainer', 'fitnessclub'),
            array_fill_keys(self::TRAINER_CAPS, true)
        );

        // add_role() is a no-op when the role exists, so re-apply caps explicitly
        // to pick up any added in a plugin update.
