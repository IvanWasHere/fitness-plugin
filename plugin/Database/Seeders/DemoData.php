...
/**
 * The prototypes' seed data, transcribed.
 *
 * Sources: user-app/index.html (SEED_DATA) and admin-app/index.html (SEED). Kept
 * faithful so a ported screen can be diffed against the prototype it came from, and
 * so every chart has realistic data on day one.
 *
 * `metric` is added where the prototype encoded the unit in a note ("Plank Hold —
 * 60 reps (seconds)", "Jump Rope — reps = jumps") — the ambiguity fc_exercises.metric
 * exists to remove.
 */
final class DemoData
{
     ...
     * Exercise row: [name, type, sets, reps, weight, rest, notes, metric?].
     *
     * @return array<int,array<string,mixed>>
     */
