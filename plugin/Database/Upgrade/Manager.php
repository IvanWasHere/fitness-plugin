 ...
 * ----------------------
 * WP Bones runs every file in database/migrations/ on activation, and dbDelta is
 * idempotent for CREATE TABLE — so *creating* tables needs nothing more. But
 * dbDelta will never drop a column, rename one, change a primary key, or backfill
 * data. Any schema change beyond "add a column" needs an explicit, ordered,
 * run-once upgrade path.
 *
 ...
 * in ascending order, exactly once per site, and record progress as they go so a
 * fatal error mid-upgrade does not re-run completed steps.
 */
final class Manager
{
    ...
    /**
     * Run any pending upgrade steps. Safe to call on every activation.
     */
    public static function run(): void
    {
        ...
        for ($version = $installed + 1; $version <= self::VERSION; $version++) {
            ...
            try {
                ...
                error_log(sprintf(
                    '[fitnessclub] Schema upgrade to v%d failed: %s',
                    $version,
                    $e->getMessage()
                ));

                // Stop at the failed step; the site stays on the last good version
                // so the next activation retries from here rather than skipping.
                return;
            }
